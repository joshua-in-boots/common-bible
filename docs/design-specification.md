...
- 상단 좌측에 3단 브레드크럼을 제공한다: 권역(구약/외경/신약) → 책 → 장.
- 브레드크럼 앞에 `index.html`(목차) 이동 버튼을 배치한다(아이콘+텍스트 “목차”).
- 레이아웃은 `.page-wrap` 래퍼 내부에서 브레드크럼/검색/본문이 좌측 정렬로 일관되게 배치된다.

### 인터랙션

- 각 단계는 드롭다운 버튼으로 동작한다.
- 모바일(폭 < 768px): 드롭다운을 바텀시트 UI로 전환하여 터치 접근성 확보.
- 패널 외부 탭 또는 닫기 버튼으로 바텀시트 닫기.

### 실제 장 목록 동기화

- “장” 드롭다운은 실제로 존재하는 장만 노출한다.
- 구현: Web Worker가 `{ type:'chapters', book }` 요청을 받아 인덱스에서 해당 책의 장 번호를 수집하여 `{ type:'chapters', book, chapters:number[] }`로 응답한다.
- 프런트는 응답을 받아 드롭다운 항목과 모바일 바텀시트 항목을 재구성한다. 워커 준비 전 요청 시에는 “로딩 중…” 표시 후 재시도한다.

자세한 메시지 규격은 [api.md](api.md)를 참고한다.
